def validate_config(cfg, vram):
    if cfg.get("bypass_validation", False):
        print("WARNING: Validation bypassed by user.")
        return cfg
    ...
    if vram["level"] == "low":
        if vid_res > 512:
            print(f"WARNING: Downgrading resolution to 512 from {vid_res} due to LOW VRAM safety.")
            cfg["video"]["resolution"] = 512
            
        if cfg["models"]["video"] != "none":
            print("WARNING: Disabling video model (low VRAM) - falling back to image sequence.")
            cfg["models"]["video"] = "none"
            
    elif vram["level"] == "medium" and vid_res > 768:
        print(f"WARNING: Downgrading resolution to 768 from {vid_res} due to MEDIUM VRAM safety.")
        cfg["video"]["resolution"] = 768

    # Example: Model Compatibility requirements
    if cfg["models"]["image"] == "sdxl" and cfg["video"]["resolution"] < 768:
        print("WARNING: SDXL degrades at low resolutions, adjusting resolution to 768...")
        cfg["video"]["resolution"] = 768
